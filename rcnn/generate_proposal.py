...
data_root = './data/round_2nd_test_b'
import json
...
def parse_args():
    ...
    parser.add_argument('--rec', help='custom image', type=str)
...
def main():
    ...
    for cls in classes:
        ...
        for idx,row in df_pred.iterrows():
            ...
            datum = [image_id,cls,int(res[0]),int(res[1]),int(res[2]),int(res[3]),res[4]]
            record.append(datum)
            #if idx >10:
             #   break
            if res[4] < 0.1:
                print(image_id)
        cloumns = ['image_id','image_category','xmin','ymin','xmax','ymax','score']
