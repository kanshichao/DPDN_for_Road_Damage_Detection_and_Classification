...
class FashionAI(IMDB):
    def __init__(self, image_set, root_path, devkit_path):
        ...
        :return: imdb object
        """
        super(FashionAI, self).__init__('FashionAI_', image_set,root_path, devkit_path)  # set self.name
        self.root_path = root_path
        self.devkit_path = devkit_path
        self.data_path = os.path.join(devkit_path,'train')
    ...
    def image_path_from_index(self, index,folder):
        """
        given image index, find out full path
        :param index: index of a specific image
        :return: full path of this image
        """
        image_file = os.path.join(self.data_path, 'JPEGImages',folder, index + '.jpg')
    ...
    def load_pascal_annotation(self, index):
        ...
        roi_rec['image'] = self.image_path_from_index(index,folder)
        ...
        if not self.config['use_diff']:
            non_diff_objs = [obj for obj in objs if int(obj.find('diffcult').text) == 0]
            objs = non_diff_objs
        num_objs = len(objs)
